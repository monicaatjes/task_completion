...
TC_BE_why_not$reason_why <-  sapply(as.character(TC_BE_why_not$reason_why), str_squish) 
TC_BE_why_not$reason_why <-  sapply(TC_BE_why_not$reason_why, first_letter_upper)

sentences <- as.character(TC_BE_why_not$reason_why)
...
s <- dplyr::left_join(translation1, TC_BE_why_not ,  by=c("reason_why"))
p1 <- dplyr::left_join(TC_BE, s, by=c("id"))  
...
TC_BEz <- dplyr::left_join(TC_BE, p_why_not, by=c("id"))
...
fail_list <- TC_BEz %>%
  ...
  dplyr::mutate(
    fail_reason = case_when(
      word1.y =="error" |word2.y =="error" ~ "technical issue",
      word1.y =="problem" |word2.y =="problem" ~ "technical issue",
      word1.y =="system" |word2.y =="system" ~ "technical issue",
      word1.y =="robot" |word2.y =="robot" | word1.y =="aborted" | word2.y =="aborted" ~ "technical issue",
      word1.y =="limit" |word2.y =="limit" ~ "reached limit",
      word2.y =="transfer" ~ "technical issue",
      word1.y =="amount" & word2.y =="too" ~ "reached limit",
      word1.y =="blocked" |word2.y =="blocked" ~ "account blocked",
      word1.y =="distracted" |word2.y =="taken"| word1.y =="pc"~ "stopped myself",
      word1.y =="account" & word2.y =="still" ~ "account not activated",
      word1.y =="after" |word2.y =="entering" ~ "login issue",
      word1.y =="apparently" & word2.y =="problem" ~ "login issue",
      word1.y =="logged" & word2.y =="logged" ~ "login issue",
      str_detect(translatedText_why, "find|looking|finding") ~ "cannot find",
      str_detect(translatedText_why, "Mishandling|naccurate account number") ~ "stopped myself",
      str_detect(translatedText_why, "sign|authoritized|itsme|logged|access|code") ~ "login issue",
      str_detect(translatedText_why, "payment|bugging|questionnaire|cut out|system|unavailable|not visible
                 |interrupted|technical malfunction|possible|failed|bad|broken|waited 18 min|inaccessible") ~ "technical issue",
      str_detect(translatedText_why, "app") ~ "problem with app",
      str_detect(translatedText_why, "cannot|Tired|waiting|rat|service|bugging|asked for an appointment|office holder
                 |stopped changing|is not possible|in an agency|sell funds
                 |new version is really not intuitive") ~ "bad service",
      str_detect(translatedText_why, "limit|Limit|Payment|limited|Amount") ~ "reached limit",
      TRUE ~ "other"
    )
  )
